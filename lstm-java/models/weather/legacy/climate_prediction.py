...
class ClimatePredictor:
                    ...
                    async with session.get(self.base_url, params=params, timeout=self.timeout) as response:
                        ...
                        if response.status == 200:
                            try:
                                ...
                                if result_code != "00":
                                    logger.error("API Error for %d-%02d: %s - %s", year, month, result_code, result_msg)
                                    return None

                                data = self._parse_weather_data(root, year, month)
                                if data:
                                    logger.info("Successfully fetched %d records for %d-%02d", len(data), year, month)
                                    return data
                                logger.warning("No data found for %d-%02d", year, month)
                            except ET.ParseError as e:
                                logger.error("XML Parse Error for %d-%02d: %s", year, month, str(e))
                        else:
                            logger.error("HTTP %d for %d-%02d", response.status, year, month)

            except asyncio.TimeoutError:
                logger.error("Timeout for %d-%02d (attempt %d)", year, month, retry + 1)
            except aiohttp.ClientError as e:
                logger.error("Client error for %d-%02d (attempt %d): %s", year, month, retry + 1, str(e))
            except ET.ParseError as e:
                logger.error("XML Parse Error for %d-%02d (attempt %d): %s", year, month, retry + 1, str(e))
            except (ValueError, KeyError, AttributeError, TypeError) as e:
                logger.error("Data processing error for %s-%02d (attempt %d): %s", year, month, retry + 1, str(e))
            except OSError as e:
                logger.error("OS or I/O error for %s-%02d (attempt %d): %s", year, month, retry + 1, str(e))
            ...
            try:
                ...
                def safe_float(value: Optional[str], field_name: str) -> float:
                    ...
                    try:
                        ...
                        logger.error("Invalid %s value: %s in %d-%02d", field_name, value, year, month)
                        return 0.0
                ...
                if not row["station"]:
                    logger.warning("Empty station name in record %d for %d-%d", idx, year, month)
                    continue

                data.append(row)

            except (ValueError, AttributeError, TypeError) as e:
                logger.error("Error parsing record %d for %d-%d: %s", idx, year, month, str(e))
                continue
    ...
    async def fetch_data_async(self, start_year: int = 2016) -> pd.DataFrame:
        ...
        logger.info("데이터 수집 기간: %d년 1월 ~ %d년 %d월", start_year, end_year, end_month)
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    ...
                    logger.error("Unexpected data type for batch %d: %s", i, type(result))
...
def main():
    ...
    try:
        ...
        if model is None:
            ...
            logger.info("Loaded existing model. Last training date: %s", last_date)

        # 예측 실행
        if last_sequence is None:
            raise ValueError("No sequence data for prediction")

        logger.info("\n=== %d Month Forecast ===", Config.PREDICTION_MONTHS)
