# File: C:\gaon\2024\weather\climate_prediction.py
...
class ClimatePredictor:
    def __init__(self):
        ...
        self.model_file = self.model_path / Config.MODEL_NAME
    ...
    def prepare_model_data(self, df, sequence_length=Config.MONTH_SEQUENCE_LENGTH):
        ...
        scaled_data = self.scaler.fit_transform(df_seoul[features])
    ...
    def load_model(self):
        """저장된 모델과 데이터 로드"""
        try:
            if not all(f.exists() for f in [self.model_file, self.sequence_file, self.last_date_file]):
...
def main():
    ...
    if model is None:
        ...
        # 모델 저장
        if not predictor.save_model(model, X[-1], last_date):
